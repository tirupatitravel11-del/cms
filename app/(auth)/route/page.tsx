...
import { Car, Megaphone, Road, User, Users } from "lucide-react";
import { usePathname, useRouter } from "next/navigation";
import { useContext, useEffect } from "react";
import VehicleTable from "@/components/tables/VehicleTable";
import Route from "@/components/tables/RouteTable";
    ...
    <>
      <div className="text-left flex bg-white p-2 border-gray-300 border rounded-md my-2 mx-auto">
        <div className="w-8 h-8 bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg flex items-center justify-center mr-3">
          <Road className="w-5 h-5 text-white" />
        </div>
        <h1 className="text-xl font-semibold text-gray-900 m-0 p-0">Route</h1>
      </div>
      <Route />
    </>
  );
