...
const VehicleType = () => {
  ...
  const [data, setData] = useState<CabType[]>([]);
  ...
  const getData = async () => {
    try {
      ...
      const res = await axios.post(
        process.env.apiUrl + "/api/all-cab-type",
        {
          limit: rowsPerPage,
          page: newPage,
          search: searchQuery,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let res = await axios.post(
        process.env.apiUrl + "/api/create-update-cab-type",
        formData,
        { withCredentials: true },
      );
      ...
      if (error?.response?.data?.error) {
        ...
        toast.error("Error in saving Cab Type");
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (id: string) => {
    try {
      let res = await axios.post(
        process.env.apiUrl + "/api/delete-cab-type",
        { id },
        { withCredentials: true },
      );
      ...
      toast.success(res?.data?.message || "Cab Type deleted successfully");
    } catch (error: any) {
      if (error?.response?.data?.error) {
        setError(error?.response?.data?.error);
        toast.error(error?.response?.data?.error);
      } else {
        setError("Error while deleting Cab Type.");
        toast.error("Error while deleting Cab Type.");
      }
    }
  };
  ...
  const handleRestore = async (id: string) => {
    try {
      let res = await axios.post(
        process.env.apiUrl + "/api/restore-cab-type",
        { id },
        { withCredentials: true },
      );
      ...
      setMessage(res?.data?.message);
      toast.success(res?.data?.message || "Cab Type restored successfully");
    } catch (error: any) {
      if (error?.response?.data?.error) {
        setError(error?.response?.data?.error);
        toast.error(error?.response?.data?.error);
      } else {
        setError("Error while restoring Cab Type.");
        toast.error("Error while restoring Cab Type.");
      }
    }
  };
  ...
  const setEditData = (item: CabType) => {
    setFormData({
      id: item._id,
      name: item.name,
